...
class FeatureEngineering(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        # IMPUTE NUMERICAL FIRST, before scaling
        if self.numerical:
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        # CRITICAL: Impute numerical features BEFORE scaling
        if numerical:
        ...
        for col in self.columns:
            ...
            if enc.strategy == "mapping":
                ...
                # CRITICAL FIX: Handle unknown values properly
                # Get a default value (either 0 or the most common mapped value)
                non_nan_values = [v for v in mapping.values() if not pd.isna(v)]
                default_value = 0.0 if non_nan_values else 0.0

                # Map values and fill NaN with default
                mapped = col_data.map(lambda x: mapping.get(x, default_value))

                # Double-check: if there are still NaNs, fill them
                if mapped.isna().any():
                ...
                # Ordinal encoder should not produce NaN, but check anyway
                ...
                # OneHot should not produce NaN, but check anyway
                if pd.DataFrame(transformed).isna().any().any():
